...
const RegexTemplates: React.FC = () => {
  ...
  const filteredGroups = groups.filter(
    (group) => group.zeroPadding != !zeroPadding,
  )
      ...
      {filteredGroups.map((group) => (
        <div className="px-4 py-1 bg-gray-50" key={group.name}>
          <div className="flex py-3 items-center justify-between text-gray-800">
            ...
            {group.name === '日付' && (
              <div
                className="flex items-center text-gray-600 font-thin text-sm cursor-pointer"
                onClick={() => setZeroPadding(!zeroPadding)}
                onKeyPress={() => setZeroPadding(!zeroPadding)}
                tabIndex={0}
                role="button"
              >
                <span className="mr-1">
                  ゼロ埋め {zeroPadding ? 'あり' : 'なし'}
                </span>
                <div
                  className={`h-4 w-9 transition duration-200 ease-in border rounded-2xl relative inline-block ${
                    zeroPadding ? 'bg-green-500' : 'bg-gray-300'
                  }`}
                >
                  <span
                    className={`absolute top-0 bg-white w-4 h-4 rounded-full ${
                      zeroPadding ? 'right-0' : 'left-0'
                    }`}
                  ></span>
                </div>
              </div>
